...
contract PackageRegistry is PackageRegistryInterface, Ownable {
    ...
    mapping(uint256 => bytes32) allPackageIds;
    // Total release number (int128) => release_id (bytes32)
    mapping(uint256 => bytes32) allReleaseIds;
    ...
    function cutRelease(
        ...
        Package storage package = packages[packageId];
        bytes32 packageReleaseId =
            generatePackageReleaseId(packageId, package.releaseCount);
    ...
    function getAllPackageIds(uint256 offset, uint256 limit)
        public
        view
        returns (bytes32[] memory packageIds, uint256 pointer)
    {
        bytes32[] memory hashes; // Array of package ids to return
        uint256 cursor = offset; // Index counter to traverse DB array
        uint256 remaining; // Counter to collect `limit` packages
    ...
    function getAllReleaseIds(
        ...
        returns (bytes32[] memory releaseIds, uint256 pointer)
    {
        bytes32 packageId = generatePackageId(packageName);
        Package storage package = packages[packageId];
        ...
        uint256 cursor = offset; // Index counter to traverse DB array
        uint256 remaining; // Counter to collect `limit` packages
        uint256 numPackageReleases = package.releaseCount; // Total number of packages in registry
        ...
        if (cursor < numPackageReleases) {
            ...
            while (remaining > 0) {
                bytes32 packageReleaseId =
                    generatePackageReleaseId(packageId, cursor);
    ...
    function numPackageIds() public view returns (uint256 totalCount) {
        return packageCount;
    }
    ...
    /// @param packageName Package Name
    function packageExists(string memory packageName)
        public
        view
        returns (bool)
    {
    ...
    {
        require(
            bytes(name).length > 2 && bytes(name).length < 255,
