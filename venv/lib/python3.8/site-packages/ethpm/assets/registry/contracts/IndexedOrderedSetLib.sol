...
library IndexedOrderedSetLib {
    struct IndexedOrderedSet {
        bytes32[] _values;
        mapping(bytes32 => uint256) _valueIndices;
    ...
    modifier requireValue(IndexedOrderedSet storage self, bytes32 value) {
        require(
            contains(self, value),
            "escape:IndexedOrderedSetLib:value-not-found"
        );
        _;
    }
    ...
    function pop(IndexedOrderedSet storage self, uint256 idx)
        public
        returns (bytes32)
    {
    ...
    function remove(IndexedOrderedSet storage self, bytes32 value)
        ...
        returns (bool)
    {
        uint256 idx = indexOf(self, value);
        pop(self, idx);
        return true;
    }
    ...
    function get(IndexedOrderedSet storage self, uint256 idx)
        public
        view
        returns (bytes32)
    {
