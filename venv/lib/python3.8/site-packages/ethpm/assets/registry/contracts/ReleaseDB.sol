...
contract ReleaseDB is Authorized {
    ...
    modifier onlyIfVersionExists(bytes32 versionHash) {
        require(
            versionExists(versionHash),
            "escape:ReleaseDB:version-not-found"
        );
        _;
    }
    ...
    function setVersion(string version) public auth returns (bytes32) {
    ...
    function getAllReleaseIds(
        ...
        uint256 _offset,
        uint256 limit
    ) public view returns (bytes32[] releaseIds, uint256 offset) {
        bytes32[] memory hashes; // Release ids to return
        uint256 cursor = _offset; // Index counter to traverse DB array
        uint256 remaining; // Counter to collect `limit` packages
        uint256 totalReleases = getNumReleasesForNameHash(nameHash); // Total number of packages in registry
    ...
    function getReleaseIdForNameHash(bytes32 nameHash, uint256 idx)
        public
        view
        returns (bytes32)
    {
    ...
    function releaseExists(bytes32 releaseId) public view returns (bool) {
    ...
    function releaseExisted(bytes32 releaseHash) public view returns (bool) {
    ...
    function versionExists(bytes32 versionHash) public view returns (bool) {
    ...
    function hashVersion(string version) public pure returns (bytes32) {
