...
contract PackageRegistry is Authorized, PackageRegistryInterface {
    ...
    function release(
        ...
        string version,
        string manifestURI
    ) public auth returns (bytes32 releaseId) {
        require(
            address(packageDb) != 0x0,
            "escape:PackageIndex:package-db-not-set"
        );
        require(
            address(releaseDb) != 0x0,
            "escape:PackageIndex:release-db-not-set"
        );
        require(
            address(releaseValidator) != 0x0,
            "escape:PackageIndex:release-validator-not-set"
        );
    ...
    /// @param name Package name
    /// @param version Version string (ex: '1.0.0')
    function releaseExists(string name, string version)
        public
        view
        returns (bool)
    {
        ...
        bytes32 versionHash = releaseDb.hashVersion(version);
        return
            releaseDb.releaseExists(
                releaseDb.hashRelease(nameHash, versionHash)
            );
    }
    ...
    function getAllPackageIds(uint256 offset, uint256 limit)
        public
        view
        returns (bytes32[] packageIds, uint256 pointer)
    {
    ...
    {
        bytes32 nameHash = packageDb.hashName(name);
        (packageOwner, createdAt, updatedAt) = packageDb.getPackageData(
            nameHash
        );
    ...
    function getAllReleaseIds(
        ...
        uint256 offset,
        uint256 limit
    ) public view returns (bytes32[] releaseIds, uint256 pointer) {
        bytes32 nameHash = packageDb.hashName(packageName);
