...
contract ReleaseValidator {
    ...
    function validateIsNewRelease(
        ...
        return
            !releaseDb.releaseExists(releaseHash) &&
            !releaseDb.releaseExisted(releaseHash);
    }

    uint256 constant DIGIT_0 = uint256(bytes1("0"));
    uint256 constant DIGIT_9 = uint256(bytes1("9"));
    uint256 constant LETTER_a = uint256(bytes1("a"));
    uint256 constant LETTER_z = uint256(bytes1("z"));
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < bytes(name).length; i++) {
            if (bytes(name)[i] == DASH && i > 0) {
                ...
                i > 0 &&
                uint256(bytes(name)[i]) >= DIGIT_0 &&
                uint256(bytes(name)[i]) <= DIGIT_9
            ) {
                continue;
            } else if (
                uint256(bytes(name)[i]) >= LETTER_a &&
                uint256(bytes(name)[i]) <= LETTER_z
            ) {
                continue;
            } else {
                return false;
    ...
    function validateStringIdentifier(string value) public pure returns (bool) {
